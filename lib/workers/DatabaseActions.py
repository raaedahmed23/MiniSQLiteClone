...
class DatabaseActions:
    ...
    def drop_database(tables,indexes):
        ...
        if os.path.exists(os.path.join(exec_path,Settings.get_data_dir())):
            tables.clear()
            indexes.clear()
            for root, directories, files in os.walk(os.path.join(exec_path,Settings.get_data_dir())):
                for file in files:
                    os.unlink(os.path.join(root, file))
                for directory in directories:
                    shutil.rmtree(os.path.join(root, directory)) # Remove the directory located at the path obtained by joining the current root directory and the subdirectory name
    ...
    @staticmethod
    # load the database state from previously saved binary files on disk 
    def load_db(tables: Dict, indexes: Dict):
        ...
        # Load System Tables and Columns
        if os.path.exists(system_tables) and os.path.exists(system_columns):
            ...
            # Execute queries to get record counts for system tables
            ...
            for t in table_names:
                    ...
                    if table_name not in tables:
                        ...
                        # Read binary data from the file and create a new_table object
