...
class Table:
    ...
    def create_table(cls, create_op: Dict, page_size=512) -> Table:
        ...
        column_list: List = create_op["column_list"]
        column_list.sort(key=lambda x: x.get("column_position", 0))
        ...
        for col in column_list:
            column_data["column_key_types"].append(col["column_key"])
            column_data["is_nullable"].append(col["is_nullable"])
            column_data["column_names"].append(col["column_name"])
            dtype_enum = DataType.from_type_name(col["data_type"])
    ...
    def table_data(self, column_data: Dict, id_row: int, name: str):
        ...
        if self.col_data_keys.difference(set(column_data.keys())) == set():
            ...
            print("Column data is missing - update failed.")

    @classmethod
    def from_table_file(cls, file_path: str) -> Table:
        byte_str = cls.read_table(file_path)
        return cls.bytes_to_object(byte_str)
    ...
    def bytes_to_object(self, byte_stream: bytes, 
        ...
        page_no = read_buff.read(page_size)
        page_num = 0
        dps: List[DataPointer] = []

        while page_no:
            page_type = self.bytes_to_int(page_no[:1])

            if page_type == 13:
                node = LeafWriter.bytes_to_object(page_no, page_num)
                leaf = node.to_bpnode()
    ...
    def object_to_bytes(self) -> bytes:
        ...
        self.tree_to_binary(self.bptree.root, 0, 0xFFFFFFFF, list)

        return b"".join(reversed(list))
    ...
    def insert(self, insert_dict: Dict) -> int:
        ...
        for i in all_columns:
            if i in insert_dict["col_ord_list"]:
                insert_values.append(insert_dict["value_list"][i])
            else:
                insert_values.append(b"")

        try:
            self.validate_insert(insert_values)
        except Exception as e:
            # traceback.print_exc()
            print('Constraint is violated: ', e)
            return
        ...
        if not self.validate_record(insertion_record):
            raise OverflowError(f"Record with values {insert_values} exceeds maximum permissible record byte size of {self.max_rec_size}")
    ...
    def update(self, update_op: Dict, condition: Dict) -> None:
        ...
        if condition and condition["column_name"] in self.indexes:
            ...
            if current_leaf is not None:
                ...
                keys_update.extend(current_leaf.keys)
        ...
        for key, rec in zip(keys_update, record_refs_all):
                key.data = rec
    ...
    def select(self, selection_dict: Dict) -> List[List]:
        ...
        column_ord_list = self.get_order_column_name_list(selection_dict["column_name_list"])
        ...
        if condition and condition["column_name"] in self.indexes:
            ...
            if current_leaf is not None:
                ...
                sels = Record.filter_subset(record_refs, column_ord_list, condition)
                for sel in sels:
            ...
            while current_leaf:
                record_ref = [key.data for key in current_leaf.keys if key.id not in self.recently_deleted]
                
                sels = Record.filter_subset(record_ref, column_ord_list, condition)
    ...
    def get_order_column_name_list(self, name_list: List[str] = []) -> List[int]:
        ...
        existing_column = self.column_data["column_names"]
        if len(name_list) == 0:
            return list(range(len(existing_column)))

        elif set(name_list).issubset(existing_column):
    ...
    def validate_insert(self, value_list: List[Any]) -> bool:
        ...
        for i, (typ, is_null, role, val) in enumerate(zip(types, nullables, col_role, value_list)):
            nv = self.validate_property(i, typ, is_null, role, val, col_names)
            value_list[i] = nv
    ...
    def validate_conditions(self, condition: Dict) -> bool:
        ...
        new_val = self.validate_property(c_ord, typ, is_null, role, val, col_names, True)
    ...
    def validate_update(self, update_op: Dict) -> bool:
        ...
        new_val = self.validate_property(c_ord, typ, is_null, role, val, col_names)
